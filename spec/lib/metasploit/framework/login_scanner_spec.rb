  ...
  describe '.all_service_names' do
    ...
    it 'returns a list of valid services' do
      ...
      all_scanners.each do |scanner|
        ...
        expect(scanner.const_defined?(:PRIVATE_TYPES)).to be(true), "scanner #{scanner} should have private types"
        expect(scanner.const_defined?(:LIKELY_SERVICE_NAMES)).to be(true), "scanner #{scanner} should have services"
        expect(scanner.const_defined?(:LIKELY_PORTS)).to be(true), "scanner #{scanner} should have ports"
        expect { scanner.new(options) }.to_not raise_error, "scanner #{scanner} should be instantiable"
      end
    end
  end
end
