  ...
  context '#try_login' do
    ...
    before :example do
      allow_any_instance_of(Rex::Proto::Http::Client).to receive(:send_recv) do |_cli, req|
        if req.opts['uri']&.include?('/users/login_exec') &&
           req.opts['data'] &&
          ...
          res
        elsif req.opts['uri']&.include?('/users/login')
        ...
        elsif req.opts['uri']&.include?('/users/login_exec')
          res = Rex::Proto::Http::Response.new(200)
          res.body = 'bad login'
        elsif   req.opts['uri']&.include?("/users/#{username}/edit")
          res = Rex::Proto::Http::Response.new(200)
  ...
  context '#attempt_login' do
    ...
    context 'when ChefWebUI' do
      ...
      it 'returns a Metasploit::Framework::LoginScanner::Result' do
        allow_any_instance_of(Rex::Proto::Http::Client).to receive(:send_recv) do |_cli, req|
          if req.opts['uri']&.include?('index.php') &&
             req.opts['data'] &&
             req.opts['data'].include?("name=#{username}") &&
             req. opts['data'].include?("password=#{password}")
            res = Rex::Proto::Http::Response.new(302)
            ...
            res
          elsif req.opts['uri']&.include?('index.php')
            res = Rex::Proto::Http::Response.new(200)
            res.body = 'bad login'
          elsif   req.opts['uri']&.include?('profile.php')
            res = Rex::Proto::Http::Response.new(200)
            res.body = 'New password for the User'
          else
            res = Rex::Proto::Http::Response.new(404)
