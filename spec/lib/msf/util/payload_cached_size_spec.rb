...
RSpec.describe Msf::Util::PayloadCachedSize do
  describe '.update_stage_sizes_constant' do
    ...
    let(:stages_with_sizes) do
      [
        OpenStruct.new(stage: OpenStruct.new(refname: 'stage_one'), size: 111),
        OpenStruct.new(stage: OpenStruct.new(refname: 'stage_two'), size: 222)
      ]
    end
    ...
    it 'replaces an existing CachedSizeOverrides value with a new one' do
      ...
      RUBY
      stages_with_sizes = [
        OpenStruct.new(stage: OpenStruct.new(refname: 'stage_new'), size: 555)
      ]
