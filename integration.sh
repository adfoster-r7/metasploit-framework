...
# Note: Initial file is duplicated and modified from Kali's test set up
# In the future more tiem can be spent converting this over to pure Ruby,
# and seeing the overlap of existing rspec tests
# https://gitlab.com/kalilinux/packages/metasploit-framework/-/blob/1971830cdcfb9c899a16f91f353b585ece3004db/debian/tests/runtests.sh

set -euf -o pipefail

AUTOPKGTEST_TMP=./tmp/integration_tests
...
test_tools() {
	...
	./tools/exploit/egghunter.rb --egg W00T --format py
	echo "Generating metasploit pattern..."
	./tools/exploit/pattern_create.rb -l 500
	echo "Locate 3Ak4 in the buffer strings..."
	./tools/exploit/pattern_offset.rb -l 500 -q 3Ak4
	echo "Testing metasm_shell utility..."

  run_metasm_shell=$AUTOPKGTEST_TMP/run_metasm_shell
  cat <<EOF > $run_metasm_shell
#!/usr/bin/expect
spawn ./tools/exploit/metasm_shell.rb
expect "metasm >"
send "add esp, 10\n"
expect "metasm >"
send "exit"
EOF
	...
	chmod 755 $run_metasm_shell
	$run_metasm_shell

  run_nasm_shell=$AUTOPKGTEST_TMP/nasm_shell
	echo -e "\nTesting nasm_shell utility..."
	cat <<EOF > $run_nasm_shell
#!/usr/bin/expect
spawn ./tools/exploit/nasm_shell.rb
expect "nasm >"
send "add esp, 10\n"
expect "nasm >"
send "exit"
...
#	msf-msu_finder -q "ms15-100" -r x86
	...
	./tools/exploit/exe2vba.rb $AUTOPKGTEST_TMP/reverse.exe $AUTOPKGTEST_TMP/reverse.vba
	echo "Testing exe2vbs.rb..."
	./tools/exploit/exe2vbs.rb $AUTOPKGTEST_TMP/reverse.exe $AUTOPKGTEST_TMP/reverse.vbs
	echo "Testing find_badchars utility..."
	./msfvenom -p windows/exec -f raw -v shellcode CMD=calc.exe EXITFUNC=thread | ./tools/exploit/find_badchars.rb -b "\x00\x0a"
