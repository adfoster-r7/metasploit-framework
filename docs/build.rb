    ...
    class Kramdown::Converter::ValidatedWikiPage < Kramdown::Converter::Html
      ...
      def convert_a(el, indent)
        ...
        if relative_link?(link_href)
          ...
          warning "Relative link present: #{link_href}, please use the syntax [[#{link_text}|#{link_href}]] instead"
        end
    ...
    def link_corrector_for(config)
      ...
      if errors.any?
        errors.each do |error|
          $stderr.puts "[!] Error found in #{error[:path]}:#{error[:messages].map { |message| "\t#{message}" }.join("\n")}"
        end
