...
module Msf::Post::Windows::Priv
  ...
  def is_uac_enabled?
    ...
    if winversion =~ /Windows (Vista|7|8|2008|2012|10|2016|2019)/ && !is_system?
      begin
  ...
  def decrypt_lsa_data(policy_secret, lsa_key)
    ...
    vprint_status("digest #{sha256x.digest.unpack1('H*')}")
  ...
  def decrypt_secret_data(secret, key)
    ...
    dec_data_len = decrypted_data[0, 4].unpack1('<L')
