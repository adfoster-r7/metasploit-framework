...
module Msf::WebServices
  ...
  # MSF_WS_DATA_SERVICE_LOG_TO_STDERR - (Boolean) Log to stderr.
  module FrameworkExtension
    ...
    def self.registered(app)
      ...
      has_stderr_logger = to_bool(ENV.fetch('MSF_WS_DATA_SERVICE_LOG_TO_STDERR', false))
      init_framework_opts = {
        # Conditionally disable the default logging functionality
        'DisableLogging' => has_stderr_logger
      }
      app.set :framework, Msf::Simple::Framework.create(init_framework_opts)
      if has_stderr_logger
        Msf::Logging.init(Rex::Logging::Sinks::Stderr.new)
      end
