  ...
  module Ui
    module Console
      ...
      module ModuleOptionTabCompletion
        ...
        def cmd_unset_tabs(mod, str, words)
          datastore = mod ? mod.datastore : self.framework.datastore
          datastore.keys
        end
        #
        ...
        def tab_complete_option(mod, str, words)
          if str.end_with?("=")
            ...
            return tab_complete_option_values(mod, option_name, words, opt: option_name).map { |value| "#{str}#{value}"}
          else
            if str.include?("=")
              str_split = str.split("=")
          ...
          ::Readline.completion_append_character = ''
          return tab_complete_option_names(mod, str, words).map { |name| "#{name}=" }
        end
        #
        # Provide tab completion for name values
        #
        def tab_complete_option_names(mod, str, words)
          res = cmd_unset_tabs(mod, str, words) || [ ]
        ...
        def option_values_payloads(mod)
          ...
          @cache_payloads = mod.compatible_payloads.map do |refname, payload|
            refname
          end
          @cache_payloads
        end
        ...
        def option_values_target_addrs(mod)
          res = [ ]
          res << Rex::Socket.source_address(mod)
        ...
        def option_values_target_ports(mod)
          res = [ ]
          return res if not framework.db.active
          return res if not mod.datastore['RHOST']
