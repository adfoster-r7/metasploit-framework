...
class RuboCopModuleRunnerException < StandardError; end
...
class RuboCopModuleRunner
  ...
  # @return [Integer] RubCop::CLI status code
  ...
  def required_modules
    ...
    if @required_modules.empty?
      raise RuboCopModuleRunnerException, 'Error retrieving new modules when verifying Rubocop'
    end
  ...
  def new_modules_for(commit)
    ...
    if !status.success? && exception
      raise RuboCopModuleRunnerException, "Command failed with status (#{status.exitstatus}): #{commit}"
    end
...
if __FILE__ == $PROGRAM_NAME
  ...
  options_parser = OptionParser.new do |opts|
    ...
    opts.on("-a", "--auto-correct", "Auto-correct offenses (only when it's safe).") do |autocorrect|
  ...
  rubocop_runner = RuboCopModuleRunner.new
  dirs.each do |dir|
    begin
      Find.find(dir) do |full_filepath|
        ...
        @exit_status = Msftidy::ERROR if rubocop_result != RuboCop::CLI::STATUS_SUCCESS
      end
    rescue Errno::ENOENT
