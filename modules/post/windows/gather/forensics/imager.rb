...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    file_data_count = 0
    disk_bytes_count = 0
    fp = ::File.new(format('%s.%03i', base_filename, file_number), 'w')
    print_line(format("Started imaging #{devname} to %s.%03i", base_filename, file_number))
    ...
    while finished != true
      ...
      file_data_count = 0
      fp = ::File.new(format('%s.%03i', base_filename, file_number), 'w')
      print_line(format('...continuing with %s.%03i', base_filename, file_number))
    end
    fp.close
