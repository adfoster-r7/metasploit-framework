...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    raise "No ports open on #{ip} from #{datastore['RPORTS']}" if !port_open
  ...
  def tcp_malformed_options_detection(_sock, ip, port)
  ...
  def tcp_dos_detection(sock, ip, _port)
    pkt = PacketFu::TCPPacket.new(config: @config)
  ...
  def icmp_code_detection(_sock, ip, _port = nil)
  ...
  def icmp_timestamp_detection(_sock, ip, _port = nil)
    pkt = PacketFu::ICMPPacket.new(config: @config)
