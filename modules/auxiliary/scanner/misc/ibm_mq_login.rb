...
class MetasploitModule < Msf::Auxiliary
  ...
  def send_userid(userid, uname)
    if datastore['PASSWORD'].nil?
      ...
      password += ("\x00" * (12 - password.length))
    end
    ...
    until username_data.empty?
      ...
      begin
        1.upto(datastore['CONCURRENCY']) do
          ...
          t << framework.threads.spawn("Module(#{refname})-#{rhost}:#{rport}", false, this_username) do |username|
            connect
            ...
            if first_response[-4..] == "\x00\x00\x00\x02" # CHANNEL_WRONG_TYPE code
