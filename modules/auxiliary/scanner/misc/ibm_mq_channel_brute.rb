    ...
    until channel_data.empty?
      ...
      begin
        1.upto(datastore['CONCURRENCY']) do
          ...
          t << framework.threads.spawn("Module(#{refname})-#{rhost}:#{rport}", false, this_channel) do |channel|
            connect
            ...
            if !data[0...3].include? 'TSH'
              next
            end
            if data[-4..] == "\x01\x00\x00\x00" # NO_CHANNEL code
              next
            end

            if data[-4..] == "\x18\x00\x00\x00" # CIPHER_SPEC code
              print_status("Found channel: #{channel}, IsEncrypted: True, IsMQI: N/A")
            elsif data[-4..] == "\x02\x00\x00\x00" # CHANNEL_WRONG_TYPE code
