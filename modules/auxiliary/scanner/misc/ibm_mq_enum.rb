...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    if chan.length > 20
      ...
      raise Msf::OptionValidateError, ['CHANNEL']
    end
      ...
      begin
        1.upto(datastore['CONCURRENCY']) do
          ...
          t << framework.threads.spawn("Module(#{refname})-#{ip}:#{this_port}", false, this_port) do |port|
            ...
            if (status_code == "\x02\x00\x00\x00") || (status_code == "\x06\x00\x00\x00")
            ...
            mq_version = data_recv[180...188].scan(/../).collect(&:to_i).join('.')
